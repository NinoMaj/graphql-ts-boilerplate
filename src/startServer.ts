...
export const startServer = async () => {
  ...
  passport.use(new Strategy({
    ...
    async (_, __, profile: any, cb: any) => {
      ...
      // if there is not user, she/he needs to register first
      if (!user) {
        ...
        // we have a twitterId this is login
  ...
  (req, res) => {
    ...
    // what ever we put in cb above will be put in req.user object
    // so we put it in session
    req.user.session = req.user.id;
  ...
  const app = await server.start({
    cors,
    port: process.env.NODE_ENV === "test" ? 4000 : 4000
  });
